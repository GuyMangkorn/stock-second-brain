...
ผู้ใช้อนุมัติ local simulation เพื่อให้พอร์ตบันทึกการซื้อขายได้โดยไม่ผูกกับ
order-capable connector. ใช้ `simulation` phase และแยก `SIMULATED_FILL` จาก
broker `FILL`; fixed decision ก่อนราคาเปิด session ถัดไปป้องกันการซื้อย้อนหลัง.
ข้อกำหนดให้มี connector ในย่อหน้าเดิมใช้เฉพาะ broker execution และไม่ขวาง
local simulation. Ledger เดิมยังเป็น canonical และไม่ถูก rewrite.
