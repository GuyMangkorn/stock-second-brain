...
| Missing data ยังไม่มี central gap ledger | Entity frontmatter มี `source_gap_count` และ `source_gaps`, แต่ยังไม่มี normalized gap records with status/priority/review_after. | Agent ตอบ "อะไร block thesis?" ได้จาก prose แต่จัดลำดับงาน follow-up ยาก. | Add `wiki/analysis/audits/Source Gap Registry.md` หรือ `raw/financials/source_gaps.json` generated from entity frontmatter and `Missing / Unverified Data` tables. |
...
| Template placeholder creates one unresolved wikilink | Local link scan found a `TICKER_fundamentals` placeholder in `wiki/reference/entity-template.md`. | This is benign, but can be escaped or documented as a template placeholder. |
