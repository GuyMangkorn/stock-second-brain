---
type: etf-performance
entity_key: Cboe BZX:DDLS
updated: 2026-07-14
return_basis: NAV total return
tags:
  - analysis/etf-performance
---

# DDLS Performance

## Bottom line

ช่วงร่วม 2021-2025 ให้ CAGR 11.33% จาก cumulative 71.05% เทียบ S&P 500 TR CAGR 14.43%; มี 1 ปีติดลบ.

## Performance check

- `entity_key: Cboe BZX:DDLS`
- Fund: WisdomTree Dynamic International SmallCap Equity Fund
- Inception: `2016-01-07`; expense ratio: `0.48%`
- Metric: `NAV Total Return` รวม distributions reinvested และ fund expenses
- Issuer benchmark: WisdomTree Dynamic International SmallCap Equity Index
- Common benchmark: `S&P 500 Total Return` (USD, dividends reinvested)
- 10-year NAV TR CAGR: `10.18%` as of `2026-06-30` (official rolling issuer figure)
- Coverage/source note: 2016 unavailable; 2017-2024 official NAV TR; 2025 secondary NAV TR*

- Annual NAV TR coverage: 2016 unavailable; 2017-2024 official NAV TR; 2025 secondary proxy*
| ปี | ETF NAV TR | S&P 500 TR |
|---|---:|---:|
| 2016 | ไม่พบข้อมูลที่ยืนยันได้ | 11.96% |
| 2017 | 25.02% | 21.83% |
| 2018 | -16.59% | -4.38% |
| 2019 | 24.74% | 31.49% |
| 2020 | -1.78% | 18.40% |
| 2021 | 16.11% | 28.71% |
| 2022 | -9.79% | -18.11% |
| 2023 | 15.16% | 26.29% |
| 2024 | 9.84% | 25.02% |
| 2025* | 29.10% | 17.88% |

**Up years / Down years**

- Best: 29.10%; least positive: 9.84%
- Worst: -9.79%; least bad down year: -9.79%
- Current YTD: `4.48%` as of `2026-06-30`

## Risk read-through

Average positive year 17.55%; worst year -9.79%. ตัวเลขสะท้อนผลของ index construction และ tracking หลังหัก fund expenses ไม่ใช่หลักฐานว่าผู้จัดการกองทุนเลือกหุ้นเชิง discretionary ได้เหนือกว่า. Classification: `dynamic international small-cap dividend`. อย่าใช้ CAGR อย่างเดียวตัดสินความเสี่ยง โดยเฉพาะ sector/country-concentrated funds.

## Sources

- [Official issuer product/performance source](https://www.wisdomtree.com/us/products/equity/ddls)
- Benchmark convention and cached 2016-2025 rows: [[ETF_performance_sources_2026-07-14]]
