...
เทียบกับ S&P 500 TR `14.43%`. Current NAV YTD: `ไม่พบข้อมูลที่ยืนยันได้`.
...
- Coverage/source note: annual rows are calendar-year NAV returns; current NAV/YTD was not disclosed in the verified Invesco capture. The 2016-2025 history includes predecessor and index-methodology changes disclosed below.
...
VIE, ADR/GDR และ currency risk. SEC prospectus ระบุว่ากองเป็น non-diversified,
ลงทุนอย่างน้อย 90% ใน index constituents/ADRs/GDRs และใช้ full replication;
management fee/total annual operating expenses ที่ระบุใน prospectus คือ
0.65%. Daily NAV history สำหรับ max drawdown และ recovery: ไม่พบข้อมูลที่
