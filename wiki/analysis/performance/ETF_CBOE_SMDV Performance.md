---
type: etf-performance
entity_key: Cboe BZX:SMDV
updated: 2026-07-14
return_basis: NAV total return
tags:
  - analysis/etf-performance
---

# SMDV Performance

## Bottom line

official calendar-year NAV Total Return ยังไม่ครบ จึงไม่จัดอันดับ common-window; ใช้ rolling return ที่ issuer เปิดเผยแยกต่างหาก.

## Performance check

- `entity_key: Cboe BZX:SMDV`
- Fund: ProShares Russell 2000 Dividend Growers ETF
- Inception: `2015-02-03`; expense ratio: `0.40%`
- Metric: `NAV Total Return` รวม distributions reinvested และ fund expenses
- Issuer benchmark: Russell 2000 Dividend Growth Index
- Common benchmark: `S&P 500 Total Return` (USD, dividends reinvested)
- 10-year NAV TR CAGR: `7.15%` as of `2026-03-31` (official rolling issuer figure)
- Coverage/source note: official calendar NAV TR table not exposed

| ปี | ETF NAV TR | S&P 500 TR |
|---|---:|---:|
| 2021 | ไม่พบข้อมูลที่ยืนยันได้ | 28.71% |
| 2022 | ไม่พบข้อมูลที่ยืนยันได้ | -18.11% |
| 2023 | ไม่พบข้อมูลที่ยืนยันได้ | 26.29% |
| 2024 | ไม่พบข้อมูลที่ยืนยันได้ | 25.02% |
| 2025 | ไม่พบข้อมูลที่ยืนยันได้ | 17.88% |

**Up years / Down years**

- Best: ไม่พบข้อมูลที่ยืนยันได้; least positive: ไม่พบข้อมูลที่ยืนยันได้
- Worst: ไม่พบข้อมูลที่ยืนยันได้; least bad down year: ไม่พบข้อมูลที่ยืนยันได้
- Current YTD: `10.38%` as of `2026-05-31`

## Risk read-through

ข้อจำกัดหลักคือ calendar-year NAV Total Return ไม่ครบ จึงยังประเมิน upside/downside รายปีอย่างเทียบเท่าไม่ได้. กองเป็น passive index-tracking equity ETF; ผลลัพธ์จึงอ่านเป็นคุณภาพของ index methodology, implementation และ cost drag. Classification: `U.S. small-cap dividend growers`. อย่าใช้ CAGR อย่างเดียวตัดสินความเสี่ยง โดยเฉพาะ sector/country-concentrated funds.

## Sources

- [Official issuer product/performance source](https://www.proshares.com/our-etfs/strategic/smdv)
- Benchmark convention and cached 2016-2025 rows: [[ETF_performance_sources_2026-07-14]]
