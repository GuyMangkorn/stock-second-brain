...
assert_contains 'Blocker routing contract'
assert_contains 'create or reuse exactly one exception card, write its complete metadata'
assert_contains 'status: ERROR'
assert_contains 'research-sub-agent-unavailable'
assert_contains 'do not check the affected queue item'
assert_contains 'batch_size` does not override a global stop'
