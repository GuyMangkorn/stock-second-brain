...
for contract_file in "$skill_file" "$prompt_file"; do
  ...
  assert_contains "$contract_file" 'If every queue item is checked and no exception is open, move the parent to `Done` and mark it complete'
  ...
  assert_contains "$contract_file" '`retryable_open`: all open non-terminal item exceptions with `confirmation: none` or `confirmation: confirmed`'
  assert_contains "$contract_file" '`selectable_retries`: entries from `retryable_open` whose ticker is not in `attempted_this_run`'
  assert_contains "$contract_file" 'Use `selectable_retries` for selection and `retryable_open` for finalization'
