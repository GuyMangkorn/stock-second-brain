...
assert_contains 'Return `global_blocked` to the manager only after the description update and lane move are each confirmed'
assert_contains 'The manager must stop and not continue to other cards after `global_blocked`'
assert_contains 'Do not claim any unconfirmed mutation'
