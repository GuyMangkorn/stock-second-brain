...
- `verification_mode: scheduled-local`
- `reviewer_dispatch: not-attempted-by-design`
- Evidence identity: the official iShares product page identifies `iShares MSCI Philippines ETF`, ticker `EPHE`, `NYSE Arca`, inception `2010-09-28`, asset class Equity, semi-annual distributions, and `34` holdings as of `2026-08-26`; latest official NAV is `$24.88` and closing market price is `$24.62` as of `2026-08-26`.
- `management_mode: passive-index`; the official prospectus and product page state that EPHE seeks to track a broad-based index composed of Philippine equities. The reviewed strategy is passive and not derivative-heavy.
- `tracked_index: MSCI Philippines IMI 25/50 Index (USD) (Net)`; `benchmark: S&P 500 Total Return` is only a common USD reference with dividends reinvested. Return basis is official `NAV Total Return` in `USD`, including reinvested distributions and after fund expenses; expense ratio is `0.59%`.
- Current official fields: the later official current-page capture reports NAV Total Return YTD `1.80%` as of `2026-08-26`, NAV `$24.88`, closing price `$24.62`, 1-day NAV change `-$0.21 (-0.84%)`, net assets `$166,674,405`, premium/discount `-1.03%`, and `34` holdings as of `2026-08-26`; 30-day SEC yield `1.88%` and 12-month trailing yield `2.69%` as of `2026-07-31`.
- Source reconciliation: an earlier official page capture showed YTD `2.66%` as of `2026-08-25`; the later official page capture showed `1.80%` as of `2026-08-26`, so the later value is selected and the earlier value is not mixed into the current snapshot.
- Official risk snapshot: P/E `9.19` and P/B `1.17` as of `2026-08-25`; 3-year standard deviation `18.24%` and beta `0.15` as of `2026-07-31`; sector exposure as of `2026-08-25` was Industrials `40.52%`, Financials `20.27%`, Utilities `11.68%`, Real Estate `10.69%`, Consumer Staples `6.67%`, Communication `4.47%`, Consumer Discretionary `3.74%`, Materials `1.01%`, Energy `0.88%`, and cash/derivatives `0.09%`.
- Official complete calendar NAV TR rows from the current factsheet/product page, through `2025-12-31`: `2021 -2.10%`, `2022 -14.37%`, `2023 -0.27%`, `2024 1.08%`, `2025 -0.54%`; no `*` or `†` markers. The corresponding index rows are `-1.44%`, `-14.05%`, `0.81%`, `2.11%`, and `-0.06%`. Annual NAV rows for `2016-2020` remain not disclosed in the reviewed U.S. factsheet table.
- Official rolling 10-year window: `2016-06-30` to `2026-06-30`, exactly `10.00` years; NAV TR cumulative `-28.05%`, CAGR `-3.24%`. Raw NAV endpoints were not disclosed; normalized representation is Start `100.00`, End `71.95` from the issuer cumulative return, not a market-price proxy.
- Calculations from displayed rows: EPHE 2021-2025 cumulative `-15.95%`, CAGR `-3.42%`, up/down years `1 / 4`, best disclosed `2024 +1.08%`, worst disclosed `2022 -14.37%`, and no full-period best/worst claim because 2016-2020 annual rows are missing. Tracked-index cumulative/CAGR are `-12.85%` / `-2.71%`; arithmetic fund-minus-index CAGR gap is `-0.70 pp` and is not called alpha. Cached S&P 500 TR for complete calendar years `2016-2025` is cumulative `96.17%`, CAGR `14.43%`, giving an arithmetic EPHE gap of `-17.85 pp`; this is only a common reference comparison.
- Index-history caveat: the issuer states that EPHE began tracking the current MSCI Philippines IMI 25/50 Index on `2020-12-01`; earlier index data is for the MSCI Philippines Investable Market Index. Current YTD is a partial period and is not used to fill missing annual history.
- Daily NAV TR observations sufficient for reproducible maximum drawdown and recovery were not captured: `ไม่พบข้อมูลที่ยืนยันได้`; no market-price drawdown or secondary proxy is substituted for NAV TR. Key gaps/risks retained are Philippines single-country, industrials/financials/utilities/real-estate concentration, FX, liquidity, policy and emerging-market sensitivity.
- Source URLs: product `https://www.ishares.com/us/products/239675/EPHE`; factsheet `https://www.ishares.com/us/literature/fact-sheet/ephe-ishares-msci-philippines-etf-fund-fact-sheet-en-us.pdf`; summary prospectus `https://www.ishares.com/us/literature/summary-prospectus/sp-ishares-msci-philippines-etf-8-31.pdf`; common-reference S&P page `https://www.spglobal.com/spdji/en/indices/equity/sp-500/`. The S&P 500 2016-2025 rows reuse the vault cache convention without a new search, with USD total-return basis, window `2016-2025`, and reference as-of `2025-12-31`.
- Complete local pre-save checklist: PASS. Identity/exchange/fund/inception, passive eligibility, tracked index, return basis/currency, fee and distribution treatment, source-date reconciliation, current and rolling as-of dates, annual-row gap and index-history break, S&P cache window, calculations, source links, unresolved gaps, Thai-first durable sections, canonical path, Philippines region assignment and graph links were checked before writing.
- Proposed durable contents: rewrite `wiki/analysis/performance/ETF_NYSE_ARCA_EPHE Performance.md` with refreshed frontmatter and sections `Bottom line`, `Performance check`, `Up years / Down years`, `Risk read-through`, and `Sources`; update the current EPHE rows in `wiki/analysis/comparisons/Philippines ETF.md` and `wiki/analysis/performance/ETF Performance Index.md` to current YTD `1.80%`; append one `etf-performance` bullet to `log.md`; retain the breadcrumb `[[ETF Region Index]] → [[Philippines ETF]] → [[ETF Performance Index]]`.
- Proposed log entry: `- etf-performance: Refreshed [[ETF_NYSE_ARCA_EPHE Performance]], [[Philippines ETF]], [[ETF Performance Index]], and extended [[ETF_performance_sources_2026-08-28]]. Scheduled-inline local pre-save returned PASS; official rolling 10-year NAV TR is -3.24% as of 2026-06-30, 2021-2025 CAGR is -3.42%, and current official NAV TR YTD is 1.80% as of 2026-08-26; EPHE remains a passive Philippines single-country ETF with industrials/financials/utilities/real-estate concentration, country/FX/liquidity sensitivity and daily NAV drawdown/recovery gap disclosed.`
- `pre_save_result: PASS`
